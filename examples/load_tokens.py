...
def load_all_tokens():
  try:
    ...
    obj_holder = [LoadAllUsers(data["user_lists"][each_user_index]["nickname"],
                           ...
                           data["user_lists"][each_user_index]["enctoken"]) for each_user_index in range(len(data["user_lists"]))]
...
class LoadAllUsers:
  ...
  def validate_users_credentials(self):
    ...
    try:
      ...
      logging.info("Error while we perform validating user : "+ self.nickname +"Error :"+ e)
